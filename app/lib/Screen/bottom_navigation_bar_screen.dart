...
class _BottomNavigationBarrScreen extends State<BottomNavigationBarrScreen> {
  int curentIndex =
      0; // Index to track the currently selected bottom navigation item.
  List<Widget> pages = [
    HomeScreen(),
    LibraryScreen(),
    ProfileScreen(),
  ...
  void _onTabTapped(int index) {
    ...
    setState(() {
      curentIndex =
          index; // Updates the current index to reflect the tapped item.
    });
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      bottomNavigationBar: BottomNavigationBar(
        ...
        selectedItemColor:
            const Color.fromARGB(255, 213, 208, 208), // Color for the selected icon and label.
